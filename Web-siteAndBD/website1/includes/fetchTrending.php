...
<table class='TriendingPages'>
	<tr>
		...
		<td style="padding: 6px 0px;"><?php echo $fetchPages['first_name']; ?> <?php echo $PageVerifyBadge; ?>
		<?php echo $fetchPages['last_name']; ?> <?php echo $PageVerifyBadge; ?>
...
$PostContentTrending = $fetch['post_content'];
    ...
    while ($fetchAuthor = $aop_trend->fetch(PDO::FETCH_ASSOC)) {
        $fetchAuthor_id = $fetchAuthor['id_uses'];
        $fetchAuthor_username = $fetchAuthor['username'];
        $fetchAuthor_fullname = $fetchAuthor['first_name'];
		$fetchAuthor_fullname = $fetchAuthor['last_name'];
    }
