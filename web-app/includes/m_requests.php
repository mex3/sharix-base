...
$users = $conn->prepare("SELECT read FROM follow WHERE reader=:myid");
...
$read = $uf['read'];
$usersInfo = $conn->prepare("SELECT id,online,first_name,last_name,userphoto,username,verify FROM signup WHERE id=:read");
$usersInfo->bindParam(':read',$read,PDO::PARAM_INT);
$usersInfo->execute();
...
<table class=\"m_contacts_table\">
	<tr class=\"mC_userLink\" data-muid=\"".$uRows['id']."\">
		...
		<td>
			<p>".$uRows['first_name']."$verifypage_var<span id=\"msgsCount\" style=\"float:".lang('float2')."\">".$mCountUnseen."</span><br><span style=\"word-break: break-word;font-size: 12px;color: #d2d2d2;\">".$lastMsg."</span></p>
			<p>".$uRows['last_name']."$verifypage_var<span id=\"msgsCount\" style=\"float:".lang('float2')."\">".$mCountUnseen."</span><br><span style=\"word-break: break-word;font-size: 12px;color: #d2d2d2;\">".$lastMsg."</span></p>
		</td>
	</tr>
</table>
";	
...
$m_from = $uf2['m_from'];
$usersInfo = $conn->prepare("SELECT id,online,first_name,last_name,userphoto,username,verify FROM signup WHERE id=:m_from");
...
$uReqCheck = $conn->prepare("SELECT read FROM follow WHERE reader=:myid AND read = :m_from");
...
$uSearch = $conn->prepare("SELECT id,online,first_name,last_name,userphoto,username,verify FROM signup WHERE id IN (SELECT read FROM follow WHERE reader= ?) AND (first_name LIKE ? OR username LIKE ?)");
...
<table class=\"m_contacts_table\">
	<tr class=\"mC_userLink\" data-muid=\"".$uSearch_row['id']."\">
		...
		<td>
			<p>".$uSearch_row['first_name']."$verifypage_var<span id=\"msgsCount\" style=\"float:".lang('float2')."\"></span><br><span style=\"font-size: 12px;color: #d2d2d2;\">@".$uSearch_row['username']."</span></p>
			<p>".$uSearch_row['last_name']."$verifypage_var<span id=\"msgsCount\" style=\"float:".lang('float2')."\"></span><br><span style=\"font-size: 12px;color: #d2d2d2;\">@".$uSearch_row['username']."</span></p>
		</td>
	</tr>
</table>
";
...
$getInfo = $conn->prepare("SELECT username,first_name,last_name,userphoto,online,bio,verify FROM signup WHERE id=:uid");
...
<h4 style='text-align: center;'><a href='".$path."u/".$getInfo_row['username']."' class='mCol3_userProfileLink'>".$getInfo_row['first_name']." ".$getInfo_row['last_name']."</a> ".$verifypage_var."</h4>
...
";
// send [userfirst_name] result in variable to push it in [json array]
$userfirst_name = $getInfo_row['first_name'];

// send [userlast_name] result in variable to push it in [json array]
$userlast_name = $getInfo_row['last_name'];
}
// [json array] to send data as array
$arr = array();
$arr[0] = $userInfo;
$arr[1] = $userInfo_bio;
$arr[2] = $userfirst_name;
$arr[3] = $userlast_name;
