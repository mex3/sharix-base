...
$notify_from = $conn->prepare("SELECT first_name,last_name,username,userphoto FROM signup WHERE id=:notify_from_id");
$notify_from->bindParam(':notify_from_id',$notify_from_id,PDO::PARAM_INT);
$notify_from->execute();
while ($from_id_row = $notify_from->fetch(PDO::FETCH_ASSOC)) {
	$first_name = $from_id_row['first_name'];
	$last_name = $from_id_row['last_name'];
