...
$db_first_name = trim(filter_var(htmlentities($_POST['first_name']),FILTER_SANITIZE_STRING));
$db_last_name = trim(filter_var(htmlentities($_POST['last_name']),FILTER_SANITIZE_STRING));
...
$uInfo = $conn->prepare("SELECT id,first_name,last_name,username,email,password,admin FROM signup WHERE username = :ed");
...
while ($uInfoRow = $uInfo->fetch(PDO::FETCH_ASSOC)) {
    ...
    $uInfo_fn = $uInfoRow['first_name'];
	$uInfo_ln = $uInfoRow['last_name'];
...
if (isset($_POST['submit_uInfo'])) {
    if (empty($db_first_name) or empty($db_last_name) or empty($db_username) or empty($db_email)) {
    ...
    if ($stop != "1") {
        ...
        $update = $conn->prepare("UPDATE signup SET first_name = :db_first_name,username = :db_username,email = :db_email,admin = :db_admin AND last_name = :db_last_name,username = :db_username,email = :db_email,admin = :db_admin WHERE username = :ed"");
        }else{
        $update = $conn->prepare("UPDATE signup SET first_name = :db_first_name,username = :db_username,email = :db_email,password = :db_password,admin = :db_admin AND last_name = :db_last_name,username = :db_username,email = :db_email,password = :db_password,admin = :db_admin WHERE username = :ed");
        }
        $update->bindParam(':db_first_name',$db_first_name,PDO::PARAM_STR);
		$update->bindParam(':db_last_name',$db_last_name,PDO::PARAM_STR);
...
if (isset($_POST['rAccBtn'])) {
    if (empty($_POST['rAccField'])) {
        ...
        $remeveAccount->bindParam(':uInfo_id',$uInfo_id,PDO::PARAM_STR);
        $remeveAccount->execute();
        $remeveAccount_sql = "DELETE FROM follow WHERE reader= :uInfo_id";
        $remeveAccount = $conn->prepare($remeveAccount_sql);
        $remeveAccount->bindParam(':uInfo_id',$uInfo_id,PDO::PARAM_STR);
        $remeveAccount->execute();
        $remeveAccount_sql = "DELETE FROM follow WHERE read= :uInfo_id";
        $remeveAccount = $conn->prepare($remeveAccount_sql);
        $remeveAccount->bindParam(':uInfo_id',$uInfo_id,PDO::PARAM_STR);
        $remeveAccount->execute();
        ...
        $remeveAccount->bindParam(':uInfo_id',$uInfo_id,PDO::PARAM_STR);
        $remeveAccount->execute();
        $remeveAccount_sql = "DELETE FROM r_star WHERE minion= :uInfo_id";
        $remeveAccount = $conn->prepare($remeveAccount_sql);
        $remeveAccount->bindParam(':uInfo_id',$uInfo_id,PDO::PARAM_STR);
        $remeveAccount->execute();
        $remeveAccount_sql = "DELETE FROM r_star WHERE favorite= :uInfo_id";
        $remeveAccount = $conn->prepare($remeveAccount_sql);
        $remeveAccount->bindParam(':uInfo_id',$uInfo_id,PDO::PARAM_STR);
        $remeveAccount->execute();
...
<div class="dashboard_box" style="text-align: <? echo lang('textAlign'); ?>">
    ...
    <form action="" method="post">
        <table>
            <tr>
                <td><? echo lang('first_name'); ?> :</td>
                <td><input dir="auto" type="text" name="first_name" class="dashboardField" value="<? echo $uInfo_fn ?>"></td>
            </tr>
			<tr>
                <td><? echo lang('last_name'); ?> :</td>
                <td><input dir="auto" type="text" name="last_name" class="dashboardField" value="<? echo $uInfo_ln ?>"></td>
            </tr>
            <tr>
