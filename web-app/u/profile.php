        ...
        <div class="profile_picture">
            ...
            <?php
               if($row_id != $_SESSION['id']){
                $csql = "SELECT id FROM follow WHERE reader=:s_id AND read=:row_id";
                $c = $conn->prepare($csql);
                ...
                $sql = "SELECT id FROM r_star WHERE minion = :uid AND favorite =:pid";
            ...
            <?php
            $firstname = $row_first_name;
            echo strtok($firstname, " ");
            ?>
			</p><hr style="margin: 0;margin-right: 10px;">
			<?php
            $lastname = $row_last_name;
            echo strtok($lasttname, " ");
            ?>
...
<!--=============================================myphotos_section==================================================-->
            ...
            $minion = $row_id;
            $emptyImg = '';
            $getphotos_sql = "SELECT * FROM wpost WHERE author_id=:minion AND post_img != :emptyImg ORDER BY post_time DESC";
            $getphotos = $conn->prepare($getphotos_sql);
            $getphotos->bindParam(':minion',$minion,PDO::PARAM_INT);
            ...
            while ($fetchMyPhotos = $getphotos->fetch(PDO::FETCH_ASSOC)) {
                ...
                while ($author_fetch = $query->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    $author_fetch_first_name = $author_fetch['first_name'];
					$author_fetch_last_name = $author_fetch['last_name'];
...
<?php
$s_id = $_SESSION['id'];
$getfollowers_sql = "SELECT * FROM follow WHERE read=:row_id";
...
<tr>
<td class='user_info_tdi'><div><img src=\"../imgs/user_imgs/".$_SESSION['userphoto']."\" alt=\"".$_SESSION['first_name']."".$_SESSION['last_name']."\" /></div></td>
<td style='width: 70%;'><a href=\"".$_SESSION['username']."\" class='user_follow_box_a'><p>".$_SESSION['first_name']."".$_SESSION['last_name']." ".$verifypage_var."<br><span style='color:gray;'>@".$_SESSION['username']."</span></a></td>
</tr>
</table>
";
}
...
$followBtn_sql = "SELECT id FROM follow WHERE reader=:s_id AND read=:id_followers";
...
<?php
$s_id = $_SESSION['id'];
$getfolloweing_sql = "SELECT * FROM follow WHERE reader=:row_id";
...
$getfolloweing_id = $getfolloweing_fetch['read'];
...
$first_name_followeing = $fetch_followeing['first_name'];
$last_name_followeing = $fetch_followeing['last_name'];
...
$followBtn_sql = "SELECT id FROM follow WHERE reader=:s_id AND read=:id_followeing";
$followBtn = $conn->prepare($followBtn_sql);
$followBtn->bindParam(':s_id',$s_id,PDO::PARAM_INT);
...
<?php
$s_id = $_SESSION['id'];
$getS_sql = "SELECT * FROM r_star WHERE favorite =:row_id";
...
while ($getS_row = $getS->fetch(PDO::FETCH_ASSOC)) {
    ...
    while ($getuser_row = $getuser->fetch(PDO::FETCH_ASSOC)) {
        ...
        $first_name_stars = $getuser_row['first_name'];
		$last_name_stars = $getuser_row['last_name'];
        $username_stars = $getuser_row['username'];
        $userphoto_stars = $getuser_row['userphoto'];
        $verify_stars = $getuser_row['verify'];
        $followBtn_sql = "SELECT id FROM follow WHERE reader=:s_id AND read=:id_stars";
        $followBtn = $conn->prepare($followBtn_sql);
        $followBtn->bindParam(':s_id',$s_id,PDO::PARAM_INT);
        ...
        <tr>
        <td class='user_info_tdi'><div><img src=\"../imgs/user_imgs/$userphoto_stars\" alt=\"$first_name_stars\" alt=\"$last_name_stars\"/></div></td>
        <td style='width: 70%;'><a href=\"$username_stars\" class='user_follow_box_a'><p>$first_name_stars $last_name_stars $verifypage_var<br><span style='color:gray;'>@$username_stars</span></a></td>
        <td style='width: 100%;'><span style='float:".lang('float2').";'>$follow_btn</span></td>
        </tr>
        </table>
        ";
