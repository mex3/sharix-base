...
$search_sql = "SELECT * FROM signup WHERE (first_name LIKE ? OR last_name LIKE ? OR country LIKE ?) OR (CONCAT(work,?,work0) LIKE ?) ORDER BY country LIMIT 8";
$params = array("%$q%", "%$q%", " ", "%$q%");
...
$csql = "SELECT id FROM follow WHERE reader=:s_id AND read=:id";
$c = $conn->prepare($csql);
...
if ($country == "") {
    $under_first_name = "@".$username;
	$under_last_name = "@".$username;
}else{
if($work == "" && $work0 == ""){
     $under_first_name = $country;
	 $under_last_name = $country;
}else{
    $under_first_name = $work0." at ".$work." . ".$country;
	$under_last_name = $work0." at ".$work." . ".$country;
}
}
if($id != $_SESSION['id']){
    $fbtn = $follow_btn;
...
<td style='width: 70%;'><a href=\"u/$username\" class='user_follow_box_a'><p>$first_name $last_name $verify_s<br><span style='color:gray;'>$under_first_name $under_last_name</span></a></td>
...
$first_name_4User = $fetch_users['first_name'];
$last_name_4User = $fetch_users['last_name'];
$username_4User = $fetch_users['username'];
$userphoto_4User = $fetch_users['userphoto'];
$verify_4User = $fetch_users['verify'];
$csql = "SELECT id FROM follow WHERE reader=:s_id AND read=:id_4User";
$c = $conn->prepare($csql);
$c->bindParam(':s_id',$s_id,PDO::PARAM_INT);
