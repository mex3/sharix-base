...
$first_name_var = filter_var(htmlentities($_POST['edit_first_name']),FILTER_SANITIZE_STRING);
$last_name_var = filter_var(htmlentities($_POST['edit_last_name']),FILTER_SANITIZE_STRING);
...
if (!password_verify($general_current_pass_var,$_SESSION['password'])) {
    ...
    if (empty($fullname_var) or empty($username_var) or empty($email_var)) {
        ...
        if (!filter_var($email_var, FILTER_VALIDATE_email)) {
         ...
         if ($stop != "1") {
         $update_info_sql = "UPDATE signup SET first_name= :first_name_var,last_name= :last_name_var,username= :username_var,email= :email_var,password= :new_password_var,gender= :gender_var WHERE username= :session_un";
         $update_info = $conn->prepare($update_info_sql);
         $update_info->bindParam(':first_name_var',$first_name_var,PDO::PARAM_STR);
		 $update_info->bindParam(':last_name_var',$last_name_var,PDO::PARAM_STR);
        ...
        if (isset($update_info)) {
			...
			$_SESSION['last_name'] = $last_name_var;
...
if (!password_verify($remeveA_current_pass_var,$_SESSION['password'])) {
    $removeA_save_result = "<p class='alertRed'>".lang('current_password_is_incorrect')."</p>";
     ...
     $remeveAccount->execute();
     $remeveAccount_sql = "DELETE FROM follow WHERE reader= :myid";
     $remeveAccount = $conn->prepare($remeveAccount_sql);
     $remeveAccount->bindParam(':myid',$myid,PDO::PARAM_STR);
     $remeveAccount->execute();
     $remeveAccount_sql = "DELETE FROM follow WHERE read= :myid";
     $remeveAccount = $conn->prepare($remeveAccount_sql);
     $remeveAccount->bindParam(':myid',$myid,PDO::PARAM_STR);
     $remeveAccount->execute();
     ...
     $remeveAccount->bindParam(':myid',$myid,PDO::PARAM_STR);
     $remeveAccount->execute();
     $remeveAccount_sql = "DELETE FROM r_star WHERE minion= :myid";
     $remeveAccount = $conn->prepare($remeveAccount_sql);
     $remeveAccount->bindParam(':myid',$myid,PDO::PARAM_STR);
     $remeveAccount->execute();
     $remeveAccount_sql = "DELETE FROM r_star WHERE favorite= :myid";
     $remeveAccount = $conn->prepare($remeveAccount_sql);
     $remeveAccount->bindParam(':myid',$myid,PDO::PARAM_STR);
     $remeveAccount->execute();
...
<?php default: ?>
    <div id="General" class="tabcontent" style="height: auto;">
        ...
        <form action="" method="post">
            <p>
            <p class="settings_fieldTitle"><?php echo lang('first_name'); ?> <span><?php echo lang('required'); ?></span></p>
            <input dir="auto" class="settings_textfield" type="text" name="edit_first_name" value="<?php echo $_SESSION['first_name']; ?>" /></p>
			<p>
            <p class="settings_fieldTitle"><?php echo lang('last_name'); ?> <span><?php echo lang('required'); ?></span></p>
            <input dir="auto" class="settings_textfield" type="text" name="edit_last_name" value="<?php echo $_SESSION['last_name']; ?>" /></p>
            <p>
